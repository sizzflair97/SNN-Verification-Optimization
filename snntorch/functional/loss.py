...
class SpikeTime(nn.Module):
    ...
    def forward(self, spk_out, targets):
        ...
        if not self.target_is_time:
            targets = self.labels_to_spike_times(targets, num_outputs, num_steps)
    ...
    def labels_to_spike_times(self, targets, num_outputs, num_steps):
        """Convert index labels [B] into spike times."""

        if not self.multi_spike:
            # targets = self.label_to_single_spike(targets, num_outputs, num_steps)
            targets_ = torch.full((targets.shape[0], num_outputs), num_steps-1)
            for i in range(targets.shape[0]):
                targets_[i, targets[i]] = 0
            targets = targets_
    ...
    def label_to_single_spike(self, targets, num_outputs, num_steps):
