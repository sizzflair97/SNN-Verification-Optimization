#!/usr/bin/env python3
"""CIFAR10-DVS verification benchmark.
...
def select_samples():
    ...
    cfg = CFG(
        log_name="probe_cifar10dvs", subtype="cifar10_dvs",
        load_data_func=load_cifar10_dvs, seed=SEED, num_samples=NUM_SAMPLES,
        deltas=(1,),
        n_layer_neurons=(INPUT_SIZE, N_HIDDEN, NUM_CLASSES),
        layer_shapes=(INPUT_SHAPE, (N_HIDDEN, 1), (NUM_CLASSES, 1)),
        num_steps=NUM_STEPS,
    )
    weights = prepare_weights(cfg=cfg, subtype="cifar10_dvs", load_data_func=load_cifar10_dvs)
    images, labels, *_ = load_cifar10_dvs(cfg)
...
def run_one(method_key, cli_args, env_overrides, delta, sample_no, run_id):
    ...
    cmd = [
        ...
        "-p", prefix, "--test-type", "cifar10_dvs",
...
def main():
    ...
    results = []
    out = RESULTS_DIR / f"cifar10dvs_bench_{run_id}.json"
