...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.AdamW(net.parameters(), lr=5e-4, betas=(0.9, 0.999))
    ...
    if not load:
        ...
        #torch.save(net, f'/Models/model_{num_hidden}.pth')
            if best_perf < (correct/total):
                torch.save(net.state_dict(), f'{location}/models/cifar_model_{num_steps}_{"_".join([str(i) for i in neurons_in_layers])}.pth')
                print("Model Saved")
                best_perf = correct/total
            scheduler.step(tot_loss)
    else:
        net_dict = torch.load(f'{location}/models/cifar_model_{num_steps}_{"_".join([str(i) for i in neurons_in_layers])}.pth')
    ...
    with open(f'{location}/models/model_accs.txt', 'a') as f:
        f.write(f"CIFAR_Rate_{num_steps}_{neurons_in_layers}: {correct/total:.3f}\n")
    #=============================================
    ...
    S = Solver()
    S.add(node_eqn)
    f = open(f'{location}/eqn/cifar_eqn_{num_steps}_{"_".join([str(i) for i in neurons_in_layers])}.txt', 'w')
    f.write(S.sexpr())
    f.close()
