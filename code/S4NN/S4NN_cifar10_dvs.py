"""S4NN training for CIFAR10-DVS (Li et al. 2017, 10 classes, 128x128 event recordings of CIFAR-10).
...
Saves weights to ../models/c10dvs_{T}_{input_size}_{H}_10/.
Usage:
  CUPY_CACHE_DIR=/tmp/cupy_cache python3 S4NN_cifar10_dvs.py \
...
from utils.load import load_cifar10_dvs
...
cfg = CFG(
    log_name="cifar10_dvs_s4nn_train",
    subtype="cifar10_dvs",
...
x_train_np, y_train_np, x_test_np, y_test_np = load_cifar10_dvs(cfg)
...
save_dir_path = osp.join(
    ROOT, f"models/c10dvs_{args.num_steps}_{input_size}_{NhidenNeurons}_{NumOfClasses}"
)
