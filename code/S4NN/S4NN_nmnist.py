"""S4NN training for N-MNIST (real DVS-recorded MNIST).

Adapted from S4NN_fmnist.py. Input is (64, 32) TTFS-encoded: two polarity
channels stacked vertically (ON rows [0:32), OFF rows [32:64)).

Saves weights to ../models/nm_{T}_{input_size}_{H}_10/.
Usage:
  CUPY_CACHE_DIR=/tmp/cupy_cache python3 S4NN_nmnist.py \
      --num-steps 5 --hidden 100 --epochs 30 [--no-gpu]
"""
...
parser.add_argument("--b", type=float, nargs=2, default=[2.0, 6.0],
                    help="upper bound of initial weights [hidden, output]; "
                         "defaults scaled for 2048-input / larger hidden")
...
# Load N-MNIST TTFS
print(f"Loading N-MNIST (T={args.num_steps}) ...")
cfg = CFG(
    log_name="nmnist_s4nn_train",
    subtype="nmnist",
    load_data_func=load_nmnist,
    seed=args.seed,
    num_samples=1,
    deltas=(1,),
    n_layer_neurons=(2 * 32 * 32, NhidenNeurons, NumOfClasses),
    layer_shapes=((64, 32), (NhidenNeurons, 1), (NumOfClasses, 1)),
    num_steps=args.num_steps,
)
x_train_np, y_train_np, x_test_np, y_test_np = load_nmnist(cfg)
print(f"train: {x_train_np.shape}, test: {x_test_np.shape}, val_range=[{x_train_np.min()},{x_train_np.max()}]")
...
save_dir_path = osp.join(ROOT, f"models/nm_{args.num_steps}_{input_size}_{NhidenNeurons}_{NumOfClasses}")
...
for epoch in tqdm(range(Nepoch), desc=f"T={args.num_steps} H={NhidenNeurons}"):
    ...
    correct = 0
    total = 0
    # Shuffle per epoch: N-MNIST loader emits samples sorted by class,
    # so without shuffling the network catastrophically forgets earlier classes.
