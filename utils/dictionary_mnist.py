...
LayerIdx = int
TimeIdx = int  # To define neuron states
InNodeIdx = tuple[int, int]
OutNodeIdx = int
InLayerIdx = int  # To define weights. The output neuron position in dimension 1 is always 1, so we ignore it.

Neuron_Layer_Time = Annotated[
    tuple[NodeIdx, LayerIdx, TimeIdx], "Represents neuron spatiotemporal position, (node, layer, timestep)."
]
...
TImageBatch = np.ndarray[tuple[Literal[60000], Literal[28], Literal[28]], np.dtype[np.int64]]
