...
def gen_spikes() -> SpkType:
    ...
    for t in range(1, num_steps+1):
        for j, m in enumerate(layers):
            for i in range(m):
                spike_indicators[(i, j, t)] = Bool(f'bSpk_{i}_{j}_{t+1}')
    return spike_indicators

def gen_inactivities() -> InactivityType:
    spike_indicators:InactivityType = InactivityType({})
    for t in range(1, num_steps+1):
        for j, m in enumerate(layers[1:], start=1):
            for i in range(m):
                spike_indicators[(i, j, t)] = Bool(f'bInAct_{i}_{j}_{t+1}')
    return spike_indicators
...
def gen_weights(weights_list:List[Tensor]):
